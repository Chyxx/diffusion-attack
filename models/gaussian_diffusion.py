...
class SD(SpacedDiffusion):
    def __init__(self, learn_sigma):
        steps = 1000
        sigma_small = False
        ...
        timestep_respacing = opt.timestep_respacing
...
class EstimatorD(SD):
    def __init__(self):
        super().__init__(learn_sigma=False)

    def compute_losses(self, estimator, classifiers, x_start, imgs, t, labels):
        # 只学hinge_loss，无须学正则化项
        noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise)
        model_output = self._wrap_model(estimator)(x_t, self._scale_timesteps(t), imgs=imgs, labels=labels)
        old_pred_arg = [c(imgs).argmax(dim=1) for c in classifiers]
        with th.enable_grad():
            x_t.requires_grad_(True)
            perts_t = norm_to_pert(x_t)
            new_pred = [c(imgs + perts_t) for c in classifiers]
            f = 0
